...
`;
  ...
  static formatMoodDetails(
    ...
    try {
      ...
      if (mood.preset) {
        message += ` preset="${this.escapeXml(mood.preset)}"`;
      }
          ...
          try {
            ...
            if (stateEntries.length > 0) {
              for (const [key, value] of stateEntries) {
                ...
                const escapedKey = this.escapeXml(String(key));
   ...
   */
  private static getMoodDetailsInstructions(): string {
    return `INSTRUCTIONS:
...
`;
  ...
  static formatDeviceInMoods(
    ...
    if (moodsWithDevice.length === 0) {
      ...
      for (const { mood, zoneName, state } of moodsWithDevice) {
        ...
        if (stateEntries.length > 0) {
          for (const [key, value] of stateEntries) {
            message += ` ${this.escapeXml(key)}="${this.escapeXml(String(value))}"`;
          }
        }
   ...
   */
  private static getDeviceInMoodsInstructions(): string {
    return `INSTRUCTIONS:
